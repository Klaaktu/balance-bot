...
\begin{itemize}
  ...
  \item The robot can smartly respond to different situations: in peaceful environments, being slightly perturbed, being strongly perturbed, commanded to move to and keep around a position. 
  \end{itemize}
\end{itemize}
...
So far, we have realized these skills: connecting and initializing each components, reading angles and accelerations from IMU, using PWM to driver motors, balancing the robot by its pitch angle with PID, controlling the yaw angle (improving), changing the PID coefficients automatically (improving), controlling the linear motion (improving). The robot of current version is shown in \figref{fig:robot_balancing_on_table}. 
...
The rough evaluations of the robot are listed in \tabref{tab:robot_evaluation_metric}. Most top-level requirements mentioned in \ref{subsec:requirements_review} are achieved. Some evaluations are even better than expected, such as the max displacement in 1s  without perturbation. However, the max allowed perturbation pitch ($<15^\circ$) is not achieved. It's because the high mass center can weaken its perturbation resistance. To improve this skills, we will try to increase the strength of motor driver, increase the wheel size, or decease the mass and the height of mass center.
